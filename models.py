...
class CrreeObservation(Observation):
    ...
    reward: float = Field(0.01, description="Reward from the step")
    done: bool = Field(False, description="Whether the episode is done")
    bug_detection: float = Field(0.01, description="Bug detection score [0-1]")
    severity_accuracy: float = Field(0.01, description="Severity accuracy score [0-1]")
    suggestion_quality: float = Field(0.01, description="Suggestion quality score [0-1]")
    security_score: float = Field(0.01, description="Security hygiene score [0-1]")
    latency_ms: float = Field(0.01, description="Step latency in milliseconds")
    memory_mb: float = Field(0.01, description="Memory usage in MB")
